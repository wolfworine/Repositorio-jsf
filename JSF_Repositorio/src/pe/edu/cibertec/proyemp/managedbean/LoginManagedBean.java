...
public class LoginManagedBean {
	...
	public String  validar() {
		...
		if (valido==null || valido.isEmpty()) {
			...
			nombrecompleto=usuario1.getFirstname() + " " + usuario1.getLastname(); 
			FacesContext context = FacesContext.getCurrentInstance();
			context.addMessage(null, new FacesMessage(
	...
	public String  cambiarpass() {
		if(pass.length()>0 && repass.length()>0){
			if(pass.equals(repass)){
				...
				loginService.getLoginRepository().update(usuario1.getId(),usuario1.getPassword(),usuario1.getEmail());
				FacesContext context = FacesContext.getCurrentInstance();
				context.addMessage(null, new FacesMessage(
						"Password de " + usuario1.getEmail()+ " actualizado"
						));
			}else{
				FacesContext context = FacesContext.getCurrentInstance();
				context.addMessage(null, new FacesMessage(
